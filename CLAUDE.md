...
詳細は `docs/WRITING_GUIDE.md` を参照。要点:

- AIっぽくない自然な口語体。セクションタイトル全部に絵文字をつけるのはNG
- 試行錯誤・ハマったポイント・失敗談を入れると人間味が出る
- 「まず」「次に」「最後に」の多用は機械的に見えるので避ける
- セクション間に水平線（`---`）を入れない。見出しで自然に区切る
- **太字**は段落内の強調。引用（`>`）は外部ソースからの引用にのみ使う
...
**目標スコア**: 7項目すべてで **4.0以上**
- 防御力、思考整理力、実践応用性、構成と読みやすさ、コミュニケーション力、炎上リスク・社会的配慮、人間らしさ
