...
正直、最初はGeminiがどこまで精度を出せるか半信半疑だったんですが、初日の通知を見たときに「ちゃんと自分が気になるやつを選んでる」と感じた。キーワードベースのフィルタリングとは違って、記事の文脈まで読んでスコアを付けてくれているのが大きい。
...
Vercel Serverless Functionsで対応しました。`api/line-webhook.ts` を置くだけでエンドポイントが生えるのが手軽。署名検証やmem0への保存ロジックは別ファイルの `handleLineWebhook()` に切り出して、Vercel側のコードはリクエストのラッパーだけにしています。
...
全部無料枠でまかなえている。フィードバックが積み上がってmem0の読み書きが増えてきたら有料プランへの切り替えを検討するつもりだけど、個人用途ではしばらく大丈夫そう。
